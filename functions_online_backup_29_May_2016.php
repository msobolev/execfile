...
function get_all_data($id='',$type='',$func = '',$from_date = '',$to_date='',$zip='',$searchnow = '',$city = '',$company = '',$industries_ids = '',$state_ids ='',$revenue = '',$employee_size = '',$display_type = '')
{
    ...
    if($searchnow != '')
    {
        ...
        if($personal_name_arr[1] != '')
            $company_personal_clause = " and (cm.company_name = '".$searchnow."' OR (first_name = '".$searched_first_name."' and last_name = '".$personal_name_arr[1]."'))";
         else    
    ...
    {    
        ...
        */
        ...
        /*
            echo "<br><br>Speaking Query: select SQL_CALC_FOUND_ROWS pm.personal_id,ps.speaking_link,first_name,last_name,
            personal_image,pm.email as email,pm.phone,speaking_id,event,event_date,
            ps.add_date as add_date,ps.role,
            ps.topic $company_related_fields
            from ".$table_personal_speaking." as ps,
    ...
    if($type == 'media' || $type == 'media_mention' || $type == 'all')
    {
        ...
        */
        ...
        $indResult = mysql_query("select SQL_CALC_FOUND_ROWS pm.personal_id,pm.phone,pmm.media_link,first_name,last_name,
            personal_image,pm.email as email,pmm.quote,pmm.publication,pmm.pub_date,
...
function get_employee_size_limits($employee_size)
{
    ...
    {    
        $indResult = mysql_query("SELECT from_range FROM hre_employee_size where id = ".$lower_revenue);
        $indRow = mysql_fetch_array($indResult);
        {
    ...
    $indResult = mysql_query("SELECT to_range FROM hre_employee_size where id = ".$upper_revenue);
    $indRow = mysql_fetch_array($indResult);
    {
...
function get_org_chart_data($company)
{
    ...
    */
    ...
    // finding max level
    $max_level = "select max(level) as max_level from ".$table_personal_master." as pm,"
            .$table_movement_master." as mm,"
            .$table_company_master." as cm
    ...
    $get_max_level = mysql_query($max_level);
    $maxRow = mysql_fetch_array($get_max_level);
    $max_level = $maxRow['max_level'];
    
    
    $get_personal = "select pm.personal_id,pm.first_name,pm.last_name,pm.level,pm.level_order,pm.personal_image,cm.company_name,mm.title from ".$table_personal_master." as pm,"
            .$table_movement_master." as mm,"
            .$table_company_master." as cm
            where  (pm.personal_id = mm.personal_id and cm.company_id = mm.company_id) and cm.company_website = '".$company."' order by level_order asc";
    ...
    $get_personal_res = mysql_query($get_personal);
