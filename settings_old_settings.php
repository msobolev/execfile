...
function this_user_invoices($user_email,$invoice_table)
{
    $invoice_dates = array();
    ...
    $find_invoices_query = mysql_query("select * from $invoice_table where email='".$user_email."'");
...
if($num_of_usr_site > 0)
{
    ...
    $get_user_q = "SELECT user_id,email from hre_user where email = '".strtolower($_SESSION['sess_email'])."'";
    ...
    {    
        ...
        $this_usr_email = $user_row['email'];

        echo "<br>this_usr_email:".$this_usr_email;
        
        $user_invoices = this_user_invoices($this_usr_email,$invoice_table);
...
{ 
    ...
    while($list_row = com_db_fetch_array($my_saved_list_res))
    {
        ...
        $list = $list_row['l_id'];
        $list_link = "http://www.execfile.com/home.php?from_date=&to_date=&type=$this_type&zip=$zip_code&city=$city&industries=$industry_ids&states=$state_ids&revenue=$revenue&employee_size=$employee_size&companyval=&title_level=$title_level&mweb=$mweb&list=$list";
...
{    
    ...
    //echo "<br>send_alerts_q: ".$send_alerts_q;
