...
function SearchResultCheckBox(totRecord){
	var contactList;
	contactList=''; 
	if(document.getElementById('checkboxCtrl').checked){
		for(i=1;i<=totRecord;i++){
			var chkBoxID='resultCheckBox_'+i;
			document.getElementById(chkBoxID).checked=true;
			if(contactList!=''){
				contactList=contactList+','+document.getElementById(chkBoxID).value;
			}else{
				contactList=document.getElementById(chkBoxID).value;
			}
		}
		document.getElementById('selected_contact_list').value=contactList;
	}else{
		for(i=1;i<=totRecord;i++){
			var chkBoxID='resultCheckBox_'+i;
...
function SelectedCheckBoxList(totRecord){
	var contactList;
	contactList='';
	for(i=1;i<=totRecord;i++){
		var chkBoxID='resultCheckBox_'+i;
	...
	if(contactList==''){
		document.getElementById('selected_contact_list').value='';
	}else{
		document.getElementById('selected_contact_list').value=contactList;
	}
}
