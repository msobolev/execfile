...
$getCompanies = "SELECT COUNT( * ) AS movementCount, cm.company_id, GROUP_CONCAT( pm.email ) personalEmails, GROUP_CONCAT(DISTINCT pm.personal_id ) as personalIds, cm.email_domain as email_domain, cm.company_website as company_website
...
{
    $personalIdsHrArr = array();
    $personalIdsCloArr = array();
    $personalIdsCmoArr = array();
    $personalIdsArr = array();
    
    
    
    $personalIds = $companyRow['personalIds'];
    $emailDomain = $companyRow['email_domain'];
    $companyWebsite = $companyRow['company_website'];
    if($emailDomain == '')
        $emailDomain = str_replace("www.",'',$companyWebsite);
    $companyId = $companyRow['company_id'];
    
    if($debug == 1)
    {    
        echo "<br><br><br><br><br>----------------------------------------------<br><br>Email Domain:".$emailDomain;
        echo "<br>Personal_emails:".$companyRow['personalEmails'];
        echo "<br>Personal_ids:".$personalIds;
        echo "<br>Company ID:".$companyId;

        echo "<br>getCtoPersonal:".$personalIds.":";
    }
    
    
    
    if($personalIds != '')
        $personalIdsArr = explode(",",$personalIds); // HR
    ...
    if($hrPersonals != '')
        $personalIdsHrArr = explode(",",$hrPersonals); // CLO
    ...
    if($debug == 1)
        echo "<br>Cto Count:".sizeof($personalIdsArr);
    if(sizeof($personalIdsArr) > 0)
    {
        foreach($personalIdsArr as $index => $personalId)
        {
            if($debug == 1)
                echo "<br>CTO personalId:".$personalId;
    ...
    if($debug == 1)
        echo "<br>HR Count:".sizeof($personalIdsHrArr);
    if(sizeof($personalIdsHrArr) > 0)
    {    
        foreach($personalIdsHrArr as $index => $personalId)
        {
            if($debug == 1)
                echo "<br>HR personalId:".$personalId;
    ...
    if($debug == 1)
        echo "<br>CLO Count:".sizeof($personalIdsHrArr);
    if(sizeof($personalIdsCloArr) > 0)
    {    
        foreach($personalIdsCloArr as $index => $personalId)
        {
            if($debug == 1)
                echo "<br>CLO personalId:".$personalId;
    ...
    if($debug == 1)
        echo "<br>CMO Count:".sizeof($personalIdsHrArr);
    if(sizeof($personalIdsCmoArr) > 0)
    {
        foreach($personalIdsCmoArr as $index => $personalId)
        {
            if($debug == 1)
                echo "<br>CMO personalId:".$personalId;
    ...
    if($debug == 1)
    {    
        echo "<pre>personalDetails:";   print_r($personalDetailsArr);   echo "</pre>";
    }
    ...
    foreach($personalDetailsArr as $index => $personalDetails)
    {
        if($debug == 1)
        {
            echo "<br><br>first_name:".$personalDetails['first_name'];
    ...
    {    
        ...
        mysql_query("insert into cto_company_update_info(page_name,query_string,add_date) values('most-likely-script','".addslashes($updateCompany)."','".date("Y-m-d:H:i:s")."')",$cto);
