...
//echo "<br>total_active_user: ".$total_active_user;
//die();
...
$download_user_lastweek = com_db_GetValue("select count(distinct(d.user_id)) from " . TABLE_DOWNLOAD." d, ".TABLE_USER." u where d.user_id=u.user_id and u.status=0 and d.add_date <='".date("Y-m-d")."' and d.add_date>='".date("Y-m-d",mktime(0,0,0,date("m"),date("d")-6,date("Y")))."'");
$download_user_lastmonth = com_db_GetValue("select count(distinct(d.user_id)) from " . TABLE_DOWNLOAD." d, ".TABLE_USER." u where d.user_id=u.user_id and u.status=0 and d.add_date<='".date("Y-m-d")."' and d.add_date>='".date("Y-m-d",mktime(0,0,0,date("m"),date("d")-30,date("Y")))."'");
