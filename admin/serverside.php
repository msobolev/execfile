...
switch($action){
	...
	case 'TodayAlertSend':
			...
			if($user_today_alert>0){
				$userQuery = "select u.user_id,u.first_name,u.last_name from ".TABLE_USER. " u,".TABLE_ALERT_SEND_INFO." asi where u.user_id = asi.user_id and DATE_FORMAT(FROM_UNIXTIME(asi.sent_date), '%Y-%m-%d')='".date("Y-m-d")."' group by asi.user_id";									
				$userResult = com_db_query($userQuery);
				if($userResult){
					$userNumRows = com_db_num_rows($userResult);
				}
				...
				while($uRow = com_db_fetch_array($userResult)){
					...
					$alertQuery = "select email_id,sent_date from ".TABLE_ALERT_SEND_INFO." where user_id='".$uRow['user_id']."' and DATE_FORMAT(FROM_UNIXTIME(sent_date), '%Y-%m-%d')='".date("Y-m-d")."' order by sent_date";									
	...
	case 'LastweekAlertSend':
			...
			if($user_lastweek_alert>0){
				$userQuery = "select u.user_id,u.first_name,u.last_name from ".TABLE_USER. " u,".TABLE_ALERT_SEND_INFO." asi where u.user_id = asi.user_id and DATE_FORMAT(FROM_UNIXTIME(asi.sent_date), '%Y-%m-%d') <='".date("Y-m-d")."' and DATE_FORMAT(FROM_UNIXTIME(asi.sent_date), '%Y-%m-%d') >='".date("Y-m-d",mktime(0,0,0,date("m"),date("d")-6,date("Y")))."' group by asi.user_id";									
				$userResult = com_db_query($userQuery);
				if($userResult){
					$userNumRows = com_db_num_rows($userResult);
				}
				...
				while($uRow = com_db_fetch_array($userResult)){
					...
					$alertQuery = "select email_id,sent_date from ".TABLE_ALERT_SEND_INFO." where user_id='".$uRow['user_id']."' and DATE_FORMAT(FROM_UNIXTIME(sent_date), '%Y-%m-%d') <='".date("Y-m-d")."' and DATE_FORMAT(FROM_UNIXTIME(sent_date), '%Y-%m-%d') >='".date("Y-m-d",mktime(0,0,0,date("m"),date("d")-6,date("Y")))."' order by sent_date";									
					$alertResult = com_db_query($alertQuery);
					$sendDateStr='';
					while($alertRow = com_db_fetch_array($alertResult)){
						if($sendDateStr==''){
	...
	case 'LastmonthAlertSend':
			...
			if($user_lastmonth_alert>0){
				$userQuery = "select u.user_id,u.first_name,u.last_name from ".TABLE_USER. " u,".TABLE_ALERT_SEND_INFO." asi where u.user_id = asi.user_id and DATE_FORMAT(FROM_UNIXTIME(asi.sent_date), '%Y-%m-%d') <='".date("Y-m-d")."' and DATE_FORMAT(FROM_UNIXTIME(asi.sent_date), '%Y-%m-%d') >='".date("Y-m-d",mktime(0,0,0,date("m"),date("d")-30,date("Y")))."' group by asi.user_id";									
				$userResult = com_db_query($userQuery);
				if($userResult){
					$userNumRows = com_db_num_rows($userResult);
				}
				...
				while($uRow = com_db_fetch_array($userResult)){
					...
					$alertQuery = "select email_id,sent_date from ".TABLE_ALERT_SEND_INFO." where user_id='".$uRow['user_id']."' and DATE_FORMAT(FROM_UNIXTIME(sent_date), '%Y-%m-%d') <='".date("Y-m-d")."' and DATE_FORMAT(FROM_UNIXTIME(sent_date), '%Y-%m-%d') >='".date("Y-m-d",mktime(0,0,0,date("m"),date("d")-30,date("Y")))."' order by sent_date";									
					$alertResult = com_db_query($alertQuery);
					$sendDateStr='';
					while($alertRow = com_db_fetch_array($alertResult)){
						if($sendDateStr==''){
