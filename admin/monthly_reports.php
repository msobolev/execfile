...
if(isset($_GET['action']) && $_GET['action'] == 'update')
{
    ...
    $ext = pathinfo($pdf_file_name, PATHINFO_EXTENSION);
    echo "<br>Ext:".$ext;
    $custom_file_name = $user_id."-report-".$month."-".$year.".".$ext;
    $destination_image = '../reports/' . $custom_file_name;
        ...
        if(is_uploaded_file($pdf_file))
        {
            ...
            if(move_uploaded_file($pdf_file , $destination_image))
            {
                //echo "<br>Within If four";
                    $insert_query = "INSERT into exec_reports(user_id,report_file,start_date,file_uploaded) values('$user_id','$custom_file_name','$start_date','1')";
...
<div style="padding-left:20px;" class="form-sing-up">
    <form action="?action=<?=$action?>&user_id=<?=$user_id?>" method="post"  enctype="multipart/form-data" onsubmit="return filter_email('field-email-1');">
    ...
    <?PHP
    $user_invoices_query = "select * from exec_reports where user_id = '".$user_id."'order by start_date desc;";
    ...
    if($invoice_count > 0)
    {
        ?>
        <br><br>
        <h2 style="padding-left:20px;">User Reports</h2>
