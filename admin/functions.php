...
function get_all_data($id='',$type='',$func = '',$from_date = '',$to_date='',$zip='',$searchnow = '',$city = '',$company = '',$industries_ids = '',$state_ids ='',$revenue = '',$employee_size = '',$title_level = '',$title_text = '',$display_type = '')
{
    ...
    if($type == 'funding' || $type == 'all' || strpos($type,"funding") > -1)
    {
        ...
        */
        ...
        $indResult = mysql_query("SELECT SQL_CALC_FOUND_ROWS personal_id,funding_source,move_id,
    ...
    if($type == 'conferences' || $type == 'all' || strpos($type,"conferences") > -1 && 1 == 2)
    { 
        ...
        $indResult = mysql_query("select SQL_CALC_FOUND_ROWS event_id,event_name,event_start_date as add_date,event_location,event_location,event_logo,event_source,status,s.short_name as short_name
        from $table_conferences as c,hre_state as s
        where c.event_state = s.state_id and $where_cluase $event_state_clause
        $from_date_clause $to_date_clause $event_site_clause
        $default_conferences_limit order by event_start_date desc");
        
        
        /*
        echo "<br>select SQL_CALC_FOUND_ROWS event_id,event_name,event_start_date as add_date,event_location,event_location,event_logo,event_source,status,s.short_name as short_name
        from $table_conferences as c,hre_state as s
        where c.event_state = s.state_id and $where_cluase $event_state_clause
        $from_date_clause $to_date_clause $event_site_clause
        $default_conferences_limit order by event_start_date desc";
        */
        ...
        $filtered_count = $num_awards_row['total_conferences_rows']+$filtered_count;
    ...
    {    
		...
		for($dm = 0; $dm < sizeof($banned_email_array); $dm++)
           ...
           // echo "<br>INSERT into ".TABLE_USER."(first_name,email,password,status,last_name,form_type,add_date,text_only,site) values('".$name."','".$email."',md5('$password'),'".$status."','".$last_name."','".$form_type."','".date("Y-m-d:H:i:s")."','".$text_only."','".$site."')";
            ...
            com_db_query("INSERT into ".TABLE_USER."(first_name,email,password,status,last_name,form_type,add_date,text_only,site) values('".$name."','".$email."',md5('$password'),'".$status."','".$last_name."','".$form_type."','".date("Y-m-d:H:i:s")."','".$text_only."','".$site."')");	    
            //die();
        }
    }    
} 
